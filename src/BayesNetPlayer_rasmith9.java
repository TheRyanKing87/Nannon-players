...
public class BayesNetPlayer_rasmith9 extends NannonPlayer {
	...
	private static int home_win[] = new int[pieces + 1]; 		// Holds p(homeX=? |  win).
	private static int home_lose[] = new int[pieces + 1]; 	// Holds p(homeX=? | !win).
	private static int safe_win[] = new int[pieces + 1];       	// NEED TO ALSO DO FOR ‘0’!
	private static int safe_lose[] = new int[pieces + 1];       	// Be sure to initialize!
	private static int board_win[][] = new int[boardSize][3]; 
	private static int board_lose[][] = new int[boardSize][3];
    ...
    int createPrimeAndHit_win = 1;
    int createPrimeAndHit_lose = 1;
    
	int bestBreakAndHit = 0;
	double bestRatio = 0;
    
	private static int board_create_prime_win[][] = new int[boardSize][3]; 
	private static int board_create_prime_lose[][] = new int[boardSize][3];
	...
	private void initialize(int cells, int pieces) {
		...
		BayesNetPlayer_rasmith9.home_win = new int[pieces + 1]; 		// Holds p(homeX=? |  win).
		BayesNetPlayer_rasmith9.home_lose = new int[pieces + 1]; 		// Holds p(homeX=? | !win).
		BayesNetPlayer_rasmith9.safe_win = new int[pieces + 1];       	// NEED TO ALSO DO FOR ‘0’!
		BayesNetPlayer_rasmith9.safe_lose = new int[pieces + 1];       	// Be sure to initialize!
		BayesNetPlayer_rasmith9.board_win = new int[boardSize][3]; 
		BayesNetPlayer_rasmith9.board_lose = new int[boardSize][3]; 
		
		BayesNetPlayer_rasmith9.board_create_prime_win = new int[boardSize][3]; 
		BayesNetPlayer_rasmith9.board_create_prime_lose = new int[boardSize][3];
	...
	public List<Integer> chooseMove(int[] boardConfiguration, List<List<Integer>> legalMoves) {
		...
		if (true && legalMoves != null) for (List<Integer> move : legalMoves) { // <----- be sure to drop the "false &&" !
			...
			double pcreatePrimeAndHit_lose = 1;
    		if (hitOpponent & createsPrimeOfMine) {
    			pcreatePrimeAndHit_lose = (double) createPrimeAndHit_lose / dWins;
    		}
	    	...
	    	double likelihoodOfWin = pHome_win * pSafe_win * pHitOpponent_win * pcreatePrimeAndHit_win * pBreakAndHit_win * pBrokeMyPrime_win * pExtendsPrimeOfMine_win * pCreatesPrimeOfMine_win * pBoard_win;				//pBreakAndHit_win
	    	double likelihoodOfLoss = pHome_lose * pSafe_lose * pHitOpponent_lose * pcreatePrimeAndHit_lose * pBreakAndHit_lose * pBrokeMyPrime_lose * pExtendsPrimeOfMine_lose * pCreatesPrimeOfMine_lose * pBoard_lose;	//pBreakAndHit_lose
