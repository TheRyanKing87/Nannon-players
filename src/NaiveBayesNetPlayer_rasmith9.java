...
public class NaiveBayesNetPlayer_rasmith9 extends NannonPlayer {
	...
	private void initialize(int cells, int pieces) {
		...
		NaiveBayesNetPlayer_rasmith9.home_win = new int[pieces + 1];	// Holds p(homeX=? |  win).
		NaiveBayesNetPlayer_rasmith9.home_lose = new int[pieces + 1]; 			// Holds p(homeX=? | !win).
		NaiveBayesNetPlayer_rasmith9.safe_win = new int[pieces + 1];       	// NEED TO ALSO DO FOR ‘0’!
		NaiveBayesNetPlayer_rasmith9.safe_lose = new int[pieces + 1];       	// Be sure to initialize!
		NaiveBayesNetPlayer_rasmith9.board_win = new int[boardSize][3]; 
		NaiveBayesNetPlayer_rasmith9.board_lose = new int[boardSize][3]; 
		int wins = 1;                            		// Remember m-estimates.
		int losses = 1;
